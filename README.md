...
- **Edge scroll** — drag the needle within 8% of either edge and the visible window scrolls smoothly underneath.
...
- **Speed cycling** — FF cycles `2×→4×→8×→16×→32×→1×`; RW cycles `−1×→−2×→−4×→−8×→−16×→−32×`.
...
| `startTime` | `JulianDate \| Date` | now − 12 h | Left bound of initial visible window |
| `endTime` | `JulianDate \| Date` | now + 12 h | Right bound of initial visible window |
...
|--------|--------|
| ⏮ | Jump to `startTime` |
| ◀◀ | Cycle reverse speeds: −1×→−2×→−4×→−8×→−16×→−32× (wraps) |
| ▶ / ⏸ | Play / Pause. If coming out of reverse, resets to 1× forward. |
| ▶▶ | Cycle forward speeds: 2×→4×→8×→16×→32×→1× (resets at end) |
| ⏭ | Jump to `endTime` |
